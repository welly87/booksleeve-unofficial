...
namespace BookSleeve
{
    ...
    public interface IStringCommands
    {
        ...
        /// </returns>
        /// <remarks>
        ///     It transpires that robust locking in redis is actually remarkably hard, and most
        ///     implementations are broken in one way or another (most commonly: thread-race, or extending the
        ///     lock duration when failing to take the lock).
        /// </remarks>
    ...
    {
        private static readonly Action<Task<bool>> takeLockContinuation =
            task =>
                {
        ...
        {
            return ExecuteVoid(RedisMessage.Create(db, RedisLiteral.SETEX, key, expirySeconds, value).ExpectOk(),
                               queueJump);
        }
        ...
        {
            return ExecuteInt64(RedisMessage.Create(db, RedisLiteral.BITOP, RedisLiteral.NOT, destination, key),
                                queueJump);
        }
        ...
        Task<bool> IStringCommands.TakeLock(int db, string key, string value, int expirySeconds, bool queueJump)
        {
            ...
            using (RedisTransaction tran = CreateTransaction())
            {
        ...
        private Task<long> IncrementImpl(int db, string key, long value, bool queueJump)
        {
            switch (value)
            {
                case 1:
        ...
        private static RedisMessage.RedisParameter[] Parse(Dictionary<string, string> values)
        {
        ...
        private static RedisMessage.RedisParameter[] Parse(Dictionary<string, byte[]> values)
        {
            if (values == null) return new RedisMessage.RedisParameter[0];
            var arr = new RedisMessage.RedisParameter[values.Count*2];
            int index = 0;
