...
namespace BookSleeve
{
    ...
    public sealed class RedisSubscriberConnection : RedisConnectionBase
    {
        ...
        public RedisSubscriberConnection(string host, int port = 6379, int ioTimeout = -1, string password = null,
        ...
        {
            get { return Interlocked.CompareExchange(ref subscriptionCount, 0, 0); }
        }
        ...
        private void ValidateKey(string key, bool pattern)
        {
            if (string.IsNullOrEmpty(key)) throw new ArgumentOutOfRangeException("key", "Empty subscription key");
            if (pattern != (key.IndexOf('*') >= 0))
                throw new ArgumentOutOfRangeException("key",
                                                      "Pattern subscriptions require *; exact subscription must not include *");
        }

        private void ValidateKeys(string[] keys, bool pattern)
        {
            if (keys == null || keys.Length == 0)
                throw new ArgumentNullException("keys", "Empty or missing set of subscription keys");
        ...
        /// <param name="handler">
        ///     A callback to invoke when messages are received on this channel;
        ///     note that the MessageReceived event will also be raised, so this callback can be null.
        /// </param>
        ...
        /// <param name="handler">
        ///     A callback to invoke when messages are received on these channel;
        ///     note that the MessageReceived event will also be raised, so this callback can be null.
        /// </param>
        /// <remarks>Channels are server-wide; they are not per-database</remarks>
        ...
        /// <param name="handler">
        ///     A callback to invoke when matching messages are received; this can be null
        ///     as the MessageReceived event will also be raised
        /// </param>
