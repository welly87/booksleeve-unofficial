...
namespace BookSleeve
{
    ...
    public interface ISortedSetCommands
    {
        ...
        Task<KeyValuePair<byte[], double>[]> Range(int db, string key, long start, long stop, bool ascending = true,
        ...
        /// <remarks>http://redis.io/commands/zrevrange</remarks>
        /// <returns>list of elements in the specified range (optionally with their scores).</returns>
        Task<KeyValuePair<string, double>[]> RangeString(int db, string key, long start, long stop,
                                                         bool ascending = true, bool queueJump = false);
        ...
        Task<long> IntersectAndStore(int db, string destionation, string[] keys,
                                     RedisAggregate aggregate = RedisAggregate.Sum, bool queueJump = false);
    ...
    partial class RedisConnection : ISortedSetCommands
    {
        ...
        Task<double>[] ISortedSetCommands.Increment(int db, string key, string[] members, double delta, bool queueJump)
        {
            ...
            var result = new Task<double>[members.Length];
            ISortedSetCommands ss = SortedSets;
        ...
        Task<KeyValuePair<byte[], double>[]> ISortedSetCommands.Range(int db, string key, long start, long stop,
                                                                      bool ascending, bool queueJump)
        {
            return
                ExecutePairs(
                    RedisMessage.Create(db, ascending ? RedisLiteral.ZRANGE : RedisLiteral.ZREVRANGE, key, start,
                                        (stop == long.MaxValue ? -1 : stop), RedisLiteral.WITHSCORES), queueJump);
        }

        Task<KeyValuePair<string, double>[]> ISortedSetCommands.RangeString(int db, string key, long start, long stop,
                                                                            bool ascending, bool queueJump)
        {
            return
                ExecuteStringDoublePairs(
                    RedisMessage.Create(db, ascending ? RedisLiteral.ZRANGE : RedisLiteral.ZREVRANGE, key, start,
                                        (stop == long.MaxValue ? -1 : stop), RedisLiteral.WITHSCORES), queueJump);
        }
        ...
        Task<long?> ISortedSetCommands.Rank(int db, string key, string member, bool ascending, bool queueJump)
        {
            return
                ExecuteNullableInt64(
                    RedisMessage.Create(db, ascending ? RedisLiteral.ZRANK : RedisLiteral.ZREVRANK, key, member),
                    queueJump);
        }
        ...
        Task<long> ISortedSetCommands.RemoveRange(int db, string key, double min, double max, bool minInclusive,
                                                  bool maxInclusive, bool queueJump)
        {
            return
                ExecuteInt64(
                    RedisMessage.Create(db, RedisLiteral.ZREMRANGEBYSCORE, key,
                                        RedisMessage.RedisParameter.Range(min, minInclusive),
                                        RedisMessage.RedisParameter.Range(max, maxInclusive)), queueJump);
        }
        ...
        /// </summary>
        [Obsolete("Please use the SortedSets API", false), EditorBrowsable(EditorBrowsableState.Never)]
        public Task<double>[] IncrementSortedSet(int db, string key, double score, string[] values,
                                                 bool queueJump = false)
        {
        ...
        /// </summary>
        [Obsolete("Please use the SortedSets API", false), EditorBrowsable(EditorBrowsableState.Never)]
        public Task<double>[] IncrementSortedSet(int db, string key, double score, byte[][] values,
                                                 bool queueJump = false)
        {
            return SortedSets.Increment(db, key, values, score, queueJump);
        }
        ...
        /// </summary>
        [Obsolete("Please use the SortedSets API", false), EditorBrowsable(EditorBrowsableState.Never)]
        public Task<KeyValuePair<byte[], double>[]> GetRangeOfSortedSetDescending(int db, string key, int start,
                                                                                  int stop, bool queueJump = false)
        {
        ...
        /// </summary>
        [Obsolete("Please use the SortedSets API", false), EditorBrowsable(EditorBrowsableState.Never)]
        public Task<KeyValuePair<byte[], double>[]> GetRangeOfSortedSet(int db, string key, int start, int stop,
                                                                        bool queueJump = false)
        {
        ...
        {
            return SortedSets.RemoveRange(db, key, start, (double) stop, queueJump);
        }
    }
}