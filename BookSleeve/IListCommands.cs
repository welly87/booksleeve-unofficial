...
namespace BookSleeve
{
    ...
    public interface IListCommands
    {
        ...
        /// <summary>
        ///     IMPORTANT: blocking commands will interrupt multiplexing, and should not be used on a connection being used by parallel consumers.
        ...
        Task<Tuple<string, string>> BlockingRemoveFirstString(int db, string[] keys, int timeoutSeconds,
        ...
        Task<byte[]> BlockingRemoveLastAndAddFirst(int db, string source, string destination, int timeoutSeconds,
                                                   bool queueJump = false);
        ...
        /// <returns>the element being popped and pushed.</returns>
        /// <remarks>http://redis.io/commands/brpoplpush</remarks>
        Task<string> BlockingRemoveLastAndAddFirstString(int db, string source, string destination, int timeoutSeconds,
                                                         bool queueJump = false);
    ...
    partial class RedisConnection : IListCommands
    {
        ...
        {
            return ExecuteInt64(RedisMessage.Create(db, RedisLiteral.LINSERT, key, RedisLiteral.BEFORE, pivot, value),
                                queueJump);
        }
        ...
        {
            return ExecuteInt64(RedisMessage.Create(db, RedisLiteral.LINSERT, key, RedisLiteral.AFTER, pivot, value),
                                queueJump);
        }
        ...
        Task<long> IListCommands.AddFirst(int db, string key, string value, bool createIfMissing, bool queueJump)
        {
            return
                ExecuteInt64(
                    RedisMessage.Create(db, createIfMissing ? RedisLiteral.LPUSH : RedisLiteral.LPUSHX, key, value),
                    queueJump);
        }
        ...
        Task<long> IListCommands.AddLast(int db, string key, string value, bool createIfMissing, bool queueJump)
        {
            return
                ExecuteInt64(
                    RedisMessage.Create(db, createIfMissing ? RedisLiteral.RPUSH : RedisLiteral.RPUSHX, key, value),
                    queueJump);
        }
        ...
        Task<string[]> IListCommands.RangeString(int db, string key, int start, int stop, bool queueJump)
        {
            return
                ExecuteMultiString(
                    RedisMessage.Create(db, RedisLiteral.LRANGE, key, start, (stop == int.MaxValue ? -1 : stop)),
                    queueJump);
        }
        ...
        Task<Tuple<string, string>> IListCommands.BlockingRemoveFirstString(int db, string[] keys, int timeoutSeconds,
                                                                            bool queueJump)
        {
            Task<string[]> msg = ExecuteMultiString(GetBlockingPop(db, RedisLiteral.BLPOP, keys, timeoutSeconds),
                                                    queueJump);
            return msg.ContinueWith(x => x.Result == null ? null : Tuple.Create(x.Result[0], x.Result[1]));
        }

        Task<Tuple<string, byte[]>> IListCommands.BlockingRemoveFirst(int db, string[] keys, int timeoutSeconds,
                                                                      bool queueJump)
        {
            Task<byte[][]> msg = ExecuteMultiBytes(GetBlockingPop(db, RedisLiteral.BLPOP, keys, timeoutSeconds),
                                                   queueJump);
            return
                msg.ContinueWith(
                    x => x.Result == null ? null : Tuple.Create(Encoding.UTF8.GetString(x.Result[0]), x.Result[1]));
        }

        Task<Tuple<string, string>> IListCommands.BlockingRemoveLastString(int db, string[] keys, int timeoutSeconds,
                                                                           bool queueJump)
        {
            Task<string[]> msg = ExecuteMultiString(GetBlockingPop(db, RedisLiteral.BRPOP, keys, timeoutSeconds),
                                                    queueJump);
            return msg.ContinueWith(x => x.Result == null ? null : Tuple.Create(x.Result[0], x.Result[1]));
        }

        Task<Tuple<string, byte[]>> IListCommands.BlockingRemoveLast(int db, string[] keys, int timeoutSeconds,
                                                                     bool queueJump)
        {
            Task<byte[][]> msg = ExecuteMultiBytes(GetBlockingPop(db, RedisLiteral.BRPOP, keys, timeoutSeconds),
                                                   queueJump);
            return
                msg.ContinueWith(
                    x => x.Result == null ? null : Tuple.Create(Encoding.UTF8.GetString(x.Result[0]), x.Result[1]));
        ...
        Task<byte[]> IListCommands.BlockingRemoveLastAndAddFirst(int db, string source, string destination,
                                                                 int timeoutSeconds, bool queueJump)
        {
            return ExecuteRaw(RedisMessage.Create(db, RedisLiteral.BRPOPLPUSH, source, destination, timeoutSeconds),
                              queueJump)
        ...
        private static RedisMessage GetBlockingPop(int db, RedisLiteral command, string[] keys, int timeoutSeconds)
        {
