...
namespace BookSleeve
{
    ...
    public interface IKeyCommands
    {
        ...
        /// </summary>
        /// <remarks>http://redis.io/commands/sort</remarks>
        Task<long> SortAndStore(int db, string destination, string key, string byPattern = null,
    ...
    partial class RedisConnection : IKeyCommands
    {
        ...
        public Task<long> Remove(int db, string[] keys, bool queueJump = false)
        {
            return
                ExecuteInt64(
                    (keys.Length == 1
                         ? RedisMessage.Create(db, RedisLiteral.DEL, keys[0])
                         : RedisMessage.Create(db, RedisLiteral.DEL, keys)), queueJump);
        }
        ...
        {
            RedisMessage msg = CreateSortMessage(db, key, byPattern, getPattern, offset, count, alpha, ascending, null);
            return ExecuteMultiString(msg, queueJump);
        }

        Task<long> IKeyCommands.SortAndStore(int db, string destination, string key, string byPattern,
                                             string[] getPattern,
                                             long offset, long count, bool alpha, bool ascending, bool queueJump)
        {
            RedisMessage msg = CreateSortMessage(db, key, byPattern, getPattern, offset, count, alpha, ascending,
                                                 destination);
