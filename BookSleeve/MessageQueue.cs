...
namespace BookSleeve
{
    ...
    {
        private readonly Queue<RedisMessage> // we'll use stdPriority as the sync-lock for both
                                             highPriority = new Queue<RedisMessage>();

        private readonly int maxSize;
        private readonly Queue<RedisMessage> stdPriority = new Queue<RedisMessage>();
        private bool closed;
        ...
        public bool TryDequeue(bool noWait, out RedisMessage value, out bool isHigh, out bool shouldFlush)
        {
            lock (stdPriority)
            {
                int timeoutMilliseconds = keepAliveMilliseconds;
