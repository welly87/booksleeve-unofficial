...
namespace BookSleeve
{
    ...
    public partial class RedisConnection : RedisConnectionBase
    {
        ...
        /// </summary>
        public RedisConnection(string host, int port = 6379, int ioTimeout = -1, string password = null,
                               int maxUnsent = int.MaxValue, bool allowAdmin = DefaultAllowAdmin,
        ...
        {
            get { return base.OutstandingCount + GetSentCount(); }
        }
        ...
        protected override void OnInitConnection()
        {
            ...
            Task<Dictionary<string, string>> options = Server.GetConfig("timeout");
            options.ContinueWith(x =>
                {
                    if (x.IsFaulted)
                    {
                        AggregateException ex = x.Exception;
                            // need to yank this to make TPL happy, but not going to get excited about it
                    }
                    else if (x.IsCompleted)
                    {
                        int timeout;
                        string text;
                        if (x.Result.TryGetValue("timeout", out text) &&
                            int.TryParse(text, NumberStyles.Any, CultureInfo.InvariantCulture, out timeout))
                        {
        ...
        public RedisSubscriberConnection GetOpenSubscriberChannel()
        {
            ...
            try
            {
                ...
                if (Interlocked.CompareExchange(ref subscriberChannel, newValue, null) == null)
                {
                    ...
                    RedisSubscriberConnection tmp = newValue;
                    newValue = null;
                    return tmp;
                }
                else
        ...
        public Counters GetCounters()
        {
            ...
            GetCounterValues(out messagesSent, out messagesReceived, out queueJumpers, out messagesCancelled, out unsent,
                             out errorMessages, out timeouts);
            return new Counters(
        ...
        internal Task<Tuple<string, int>> QuerySentinelMaster(string serviceName)
        {
            ...
            var taskSource = new TaskCompletionSource<Tuple<string, int>>();
            ExecuteMultiString(RedisMessage.Create(-1, RedisLiteral.SENTINEL, "get-master-addr-by-name", serviceName),
                               false, taskSource)
                .ContinueWith(task =>
                    {
                        ...
                        if (Condition.ShouldSetResult(task, state))
                        {
                            ...
                            {
                                state.SetException(
                                    new InvalidOperationException("Invalid sentinel result: " + string.Join(",", arr)));
                            }
                        }
                    });
